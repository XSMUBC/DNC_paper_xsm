            ...
            for step in range(hp.steps):
                ...
                if step % 100 == 0:
                    ...
                    print("Summary generated. Step", step,
                          " Test cost == %.9f Time == %.2fs" % (cost_value, time() - t))  # xsm xsm test cost and time cost 
                    t = time()


                    outputnum=1000
                    print("Output number is ", outputnum)
                    if step % outputnum == 0 and step > 0:
                        task.test(sess, outputs, [x, y, sequence_lengths, mask], hp.batch_size)
                        saver.save(sess, project_path.model_path)
                        print("Model saved!")  #   5000 is the output number 
