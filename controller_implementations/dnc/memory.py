...
class Memory:
    ...
    def step(self, controller_output, memory_state, cslm):
        """
        ...
        wrration=int(self.word_size/self.num_read_heads)
        ...
        cslm=cslm+cslm_ratio1*tf.nn.softmax(tf.einsum("bn,bw->bnw", write_weighting, interf["write_vector"]), dim=2)+cslm_ratio2*tf.nn.softmax(m, dim=2)+cslm_ratio3*tf.nn.softmax(tf.tile(read_weightings,multiples=[1, 1, wrration]), dim=2)
        #print("cslm_ratio1,cslm_ratio2,cslm_ratio3", cslm_ratio1,cslm_ratio2,cslm_ratio3)
#        cslm=tf.nn.l2_normalize(cslm, dim=2)
#        cslm=tf.nn.l2_normalize(cslm, dim=1)
#        cslm=tf.nn.l2_normalize(cslm, dim=0)
#        cslm=tf.nn.softmax(cslm, dim=2)
#        cslm=tf.nn.softmax(cslm, dim=1)
#        cslm=tf.nn.softmax(cslm, dim=0)
        # memory retention is of shape [batch_size, memory_size]

        #image_recon = self.to_image(cslm)  # reconstructure image

        #print('xsm image_recon',image_recon.size())
        ...
        allocation_weighting = self.calculate_allocation_weighting(usage_vector,cslm)
        ...
        write_content_weighting = Memory.content_based_addressing(m, interf["write_key"], interf["write_strength"],cslm)
    ...
    def calculate_allocation_weighting(self, usage_vector,cslm):
        """
        ...
        cslm2=tf.reduce_sum(cslm,2)

#        cslm=tf.nn.softmax(cslm, dim=1)
        cslm_weight1=0.08
        usage_vector=usage_vector+cslm_weight1*cslm2
        #print("cslm_weight1", cslm_weight1)
#        usage_vector=usage_vector*0.05*cslm
#        usage_vector=usage_vector+0.1*cslm
    ...
    @staticmethod
    def content_based_addressing(memory, keys, strength,cslm):
        """
        ...
        cslm_weight2=0.08
        memory=memory+cslm_weight2*cslm
        #print("cslm_weight2", cslm_weight2)
#        memory=memory*0.05*cslm
